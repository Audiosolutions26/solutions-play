...
export function QuickStartPanel({ embedded }: { embedded?: boolean } = {}) {
  ...
  const update = (id: string, patch: Partial<Pad>) =>
    setPads((ps) => ps.map((p) => (p.id === id ? { ...p, ...patch } : p)));
  ...
  const addPad = () => {
    ...
    setPads((ps) => [
      ...ps,
      { id, label: "Novo botão", freq: 300, dur: 1, color: PALETTE[ps.length % PALETTE.length] },
    ]);
  };
  ...
  const onPickFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      update(id, {
        url,
        dur: Math.max(0.5, Math.min(duration || 1, 8)),
      ...
      <div className="pl-scroll flex-1 overflow-y-auto bg-pl-row p-3">
        ...
        >
          {pads.map((p) => {
            ...
            return (
              ...
              >
                ...
                >
                  {p.url ? (
                    <FileAudio className="h-7 w-7 drop-shadow" />
                  ) : (
                    <Zap className="h-7 w-7 drop-shadow" />
                  )}
                  <span className="px-1 text-[12px] font-bold leading-tight drop-shadow">
                    {p.label}
                  </span>
                  {p.key && (
                    <span className="absolute right-1.5 top-1.5 rounded bg-black/25 px-1 text-[10px] font-mono">
                      {p.key}
                    </span>
                  )}
                  {p.random && (
                    <Shuffle className="absolute left-1.5 top-1.5 h-3.5 w-3.5 opacity-80" />
                  )}
                  {isActive && (
                    <span className="absolute inset-x-2 bottom-2 h-1 overflow-hidden rounded bg-black/20">
                      <span
                        className="block h-full w-full origin-left animate-[ql_var(--d)_linear] bg-white/80"
                        style={{ ["--d" as string]: `${p.dur}s` }}
                      />
                    </span>
                  )}
                </button>
                {editMode && (
                  <button
                    onClick={() => remove(p.id)}
                    title="Excluir botão"
                    className="absolute -right-2 -top-2 grid h-5 w-5 place-items-center rounded-full bg-destructive text-white shadow"
                  >
                    <X className="h-3 w-3" />
                  </button>
                )}
                {editMode && (
                  <span className="pointer-events-none absolute left-1.5 top-1.5 z-10 text-white/80">
                  ...
                  <div className="mt-1 space-y-1 rounded border border-pl-panel-dark/30 bg-white/70 p-1.5 text-[10px]">
                    ...
                    <div className="flex items-center gap-1">
                      <button
                        ...
                        className="grid h-6 flex-1 place-items-center rounded bg-muted hover:brightness-95"
                      >
                    ...
                    <div className="flex items-center gap-1">
                      <select
                        value={p.key ?? ""}
                        onChange={(e) => update(p.id, { key: e.target.value || undefined })}
                        className="flex-1 rounded border border-pl-panel-dark/40 px-1 py-0.5"
                      >
          ...
          {editMode && (
            <button
              onClick={addPad}
              className="flex aspect-square items-center justify-center rounded-lg border-2 border-dashed border-pl-panel-dark/40 text-pl-text/60 hover:bg-white/40"
            >
