...
export function TextoDoDiaPanel() {
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem(DAYTEXT_STORE);
      if (raw) {
        const parsed: DayText[] = JSON.parse(raw);
        setItems(parsed);
  ...
  const persist = (next: DayText[]) => {
    setItems(next);
    try { localStorage.setItem(DAYTEXT_STORE, JSON.stringify(next)); } catch { /* ignore */ }
  };
  ...
  const save = () => {
    ...
    if (active) {
      persist(items.map((i) => (i.id === active.id ? { ...i, title: title || i.title, body } : i)));
    } else {
      const it: DayText = { id: `dt${Date.now()}`, date, title: title || "Texto do dia", body };
      persist([it, ...items]);
      setActiveId(it.id);
    }
  };

  const open = (it: DayText) => { setActiveId(it.id); setTitle(it.title); setBody(it.body); };
  const del = (id: string) => {
        ...
        <div className="pl-scroll flex w-1/3 flex-col overflow-y-auto border-r border-pl-panel-dark/30 bg-pl-row">
          ...
          {items.map((t) => (
            <button
              key={t.id}
              onClick={() => open(t)}
              className={`flex items-center justify-between gap-1 border-b border-pl-panel-dark/20 px-2 py-1.5 text-left text-[12px] ${activeId === t.id ? "bg-pl-toolbar-light/40 font-semibold" : "hover:bg-muted"}`}
            >
              <span className="min-w-0">
                <span className="block truncate">{t.title}</span>
                <span className="block text-[10px] text-muted-foreground">{t.date}</span>
              </span>
              <Trash2 className="h-3 w-3 shrink-0 text-destructive opacity-60 hover:opacity-100" onClick={(e) => { e.stopPropagation(); del(t.id); }} />
            </button>
          ))}
