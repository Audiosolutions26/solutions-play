...
export function FoldersPanel({
  ...
  const matchesLibraryFilter = (track: Track) =>
    libraryFilter === "all" || track.category === libraryFilter;
  const filterTracks = (tracks: Track[]) => tracks.filter(matchesLibraryFilter);
  ...
  const results = useMemo(() => {
    ...
    return folders.flatMap((f) =>
      f.tracks.filter(
        (t) =>
          (libraryFilter === "all" || t.category === libraryFilter) &&
          (t.title.toLowerCase().includes(q) || (t.artist ?? "").toLowerCase().includes(q)),
      ),
    );
  }, [query, folders, libraryFilter]);
  ...
  const preview = (t?: Track | null) => {
    const track = t ?? sel;
    if (!track) {
      toast.info("Selecione um áudio para pré-escuta.");
      return;
    }
    setSel(track);
    void cuePlay(track);
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="flex items-center gap-1 border-b border-pl-toolbar-dark bg-pl-panel-dark px-2 py-1">
        ...
        <button
          onClick={() => preview()}
          title="Pré-escuta (atalho: C)"
          className="grid h-7 w-7 place-items-center rounded bg-gradient-to-b from-pl-transport to-pl-transport-dark text-white hover:brightness-110"
        >
          <Play className="h-4 w-4" />
        </button>
        <button
          onClick={stopPreview}
          title="Parar pré-escuta (atalho: C)"
          className="grid h-7 w-7 place-items-center rounded bg-gradient-to-b from-pl-transport to-pl-transport-dark text-white hover:brightness-110"
        >
      ...
      <div className="pl-scroll flex-1 overflow-y-auto bg-pl-row p-2">
        {results ? (
          ...
          <TrackList
            tracks={filterTracks(open.tracks)}
            onAdd={add}
            onSelect={setSel}
            selId={sel?.id}
            onPreview={preview}
          ...
          <div className="space-y-3">
            {SHORTCUT_TYPES.map((meta) => {
              const items = folders.filter(
                (f) => (f as FolderType & { type?: string }).type === meta.type,
              );
      ...
      <div className="flex flex-wrap items-center gap-2 border-t border-pl-panel-dark bg-pl-panel px-2 py-1.5">
        ...
        >
          ...
          <select
            ...
            className="max-w-[92px] bg-transparent text-[11px] outline-none"
          >
            <option value="all">Todos</option>
            <option value="musical">Músicas</option>
            <option value="vinheta">Vinhetas</option>
...
function TrackList({
  ...
  if (!tracks.length)
    return <p className="p-4 text-center text-[12px] text-muted-foreground">Nada encontrado.</p>;
  return (
        ...
        return (
          <ContextMenu key={t.id}>
            <ContextMenuTrigger asChild>
              ...
              >
                {isCue ? (
                  <Headphones className="h-3.5 w-3.5 shrink-0 text-emerald-600" />
                ) : (
